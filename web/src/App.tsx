...
import { api, Surface, SourceFile, Status } from './api';
...
export function App() {
  ...
  const handlePointerUp = () => {
    if (dragging !== null && surface) {
      // Send updated points to backend
      api.surfaces.update(surface.id, { dst_points: surface.dst_points });
    }
    setDragging(null);
  };
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 16 }}>
      ...
      <div style={{ position: 'relative', marginBottom: 16, background: '#111', borderRadius: 8, overflow: 'hidden' }}>
        ...
        <canvas
          ...
          style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', cursor: dragging !== null ? 'grabbing' : 'crosshair' }}
        />
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, fontSize: 11, color: '#aaa' }}>
            {['Oben Links', 'Oben Rechts', 'Unten Rechts', 'Unten Links'].map((label, i) => (
              <div key={i} style={{ background: '#1a1a1a', padding: 8, borderRadius: 4, textAlign: 'center' }}>
                ...
                <div>X: {surface.dst_points[i]?.[0]}</div>
                <div>Y: {surface.dst_points[i]?.[1]}</div>
              </div>
            ))}
          </div>
          <button
